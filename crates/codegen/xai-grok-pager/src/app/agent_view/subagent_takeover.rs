...
impl AgentView {
    ...
    /// Rung order is fixed: Ctrl+Q, `[✗]` click, session-id click, hover, `c` copy session, bare-scrollback close,
    /// idle-quote, forward + filter.
    pub(super) fn intercept_takeover_input(
        ...
        if key.is_some_and(|key| key!('c').matches(key))
            && self
                .subagent_views
                .get_mut(&child_sid)
                .is_some_and(|child| child.copy_session_id_to_clipboard())
        {
            return Some(InputOutcome::Changed);
        }
